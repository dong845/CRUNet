...
class CRUNet_D_Mon_Block(nn.Module):
    def __init__(self, c_iters, chans=64, norms=True, tcp=False):
        super().__init__()
        self.chans = chans
        self.chans1 = chans
        self.norms = norms
        ...
        self.conv11 = nn.Conv2d(self.chans, self.chans, 3, 1, 1)
        self.conv21 = nn.Conv2d(self.chans1, self.chans1, 3, 1, 1)
        self.conv31 = nn.Conv2d(self.chans1, self.chans1, 3, 1, 1)
        self.conv41 = nn.Conv2d(self.chans1, self.chans1, 3, 1, 1)
        self.conv51 = nn.Conv2d(self.chans1, self.chans1, 3, 1, 1)
        ...
        if c_iters>0:
            self.mon_convs = nn.ModuleList([
                nn.Conv2d(chans*c_iters, chans, 3, 1, dilation=4, padding=4),
                nn.Conv2d(chans*c_iters, chans, 3, 1, dilation=2, padding=2),
                nn.Conv2d(chans*c_iters, chans, 3, 1, 1),
            ])
    ...
    def forward(self, ref_kspace, x, net, mask, sens_maps, lls): 
        b, ch, h, w, t = x.size()       
        indices = list(range(12))
        if self.norms:
            x = x.permute(0,1,4,2,3).contiguous()
        ...
        for key in lls.keys():
            if len(lls[key])>0:
                net_tmp = torch.cat(lls[key], dim=1)
                net[key] = self.mon_convs[times](net_tmp)
                if key=="x3":
                    net['x1'] = net['x3']
                elif key=="x4":
                    net['x0'] = net['x4']
        ...
        net['x2'] = self.bcrnn(net['x1'].reshape(t, b, self.chans1, h//2, w//2), net['x2'])+self.conv31(net['x1'].reshape(-1, self.chans1, h//2, w//2)).reshape(t, b, self.chans1, h//2, w//2)
        net['x2'] = self.convs2(net['x2'].reshape(-1, self.chans, h//2, w//2))
        lls['x2'].append(net['x2'])

        net['x3'] = net['x3'].view(t, b, self.chans1, h//2, w//2)
        net['x3'] = self.u_crnn1(net['x2'].reshape(t, b, self.chans1, h//2, w//2), net['x3'])+self.conv41(net['x2'].reshape(-1, self.chans1, h//2, w//2)).reshape(t, b, self.chans1, h//2, w//2)
        net['x3'] = torch.cat((net['x1'].reshape(-1, self.chans1, h//2, w//2), net['x3'].reshape(-1, self.chans1, h//2, w//2)), dim=1)
        net['x3'] = self.conv1(net['x3'])
        lls['x3'].append(net['x3'])
...
class CRUNet_D_Mon_NWS(nn.Module):
    def __init__(self, num_cascades=5, chans=64):
        super().__init__()
        self.num_cascades = num_cascades
        self.chans = chans
        self.chans1 = chans
        self.cascades = nn.ModuleList(
            [CRUNet_D_Mon_Block(c_iters=i, chans=chans) for i in range(self.num_cascades)]
        )
    ...
    def forward(self, ref_kspace, mask, sens_maps):
        ...
        for cascade in self.cascades:
            x, net, lls = cascade(ref_kspace, x, net, mask, sens_maps, lls)
        return x.permute(0,4,2,3,1).contiguous()
